...
public interface CargoInfoRepository extends JpaRepository<CargoInfo, Long>, JpaSpecificationExecutor<CargoInfo> {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Modifying
    @Query(value = "update cargo_info set is_delete=1 where cargo_id in ?1 and status=1", nativeQuery = true)
    ...
    @Query(value = "select * from cargo_info c where c.is_delete=2 " +
            "and c.cargo_id= (select e.cargo_id from part_info_exp e where  e.project_id=?1 limit 1) " , nativeQuery = true)
