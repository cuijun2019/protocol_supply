...
public interface InquiryInfoRepository extends JpaRepository<InquiryInfo, Long>, JpaSpecificationExecutor<InquiryInfo> {
    ...
    @Modifying
    @Query(value = "update inquiry_info set is_delete=1 where inquiry_id in ?1", nativeQuery = true)
    ...
    @Query(value = "select i.* from inquiry_info i where exists(select 1 from busi_jbpm_flow b where i.inquiry_id = b.business_id and b.business_type='enquiryAudit' and read_type is null " +
            ...
            "if((?2 is not null), (exists (select 1 from cargo_info c where c.cargo_id = i.cargo_id and c.cargo_name like %?2%)), (1=1)) order by i.inquiry_date desc", nativeQuery = true)
