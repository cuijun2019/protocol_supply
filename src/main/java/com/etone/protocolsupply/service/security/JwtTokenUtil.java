...
public class JwtTokenUtil implements Serializable {
    ...
    public String generateToken(UserDetails userDetails) {
        ...
        if ((grantedAuthorities != null) && (!grantedAuthorities.isEmpty())){
            claims.put(CLAIM_KEY_ROLE,((GrantedAuthority)grantedAuthorities.toArray()[0]).getAuthority());
            if (userDetails instanceof JwtUser){
                JwtUser user = (JwtUser) userDetails;
                claims.put(CLAIM_KEY_FULLNAME, StringUtils.isEmpty(user.getFullname()) ? userDetails.getUsername() : user.getFullname());
            }
        }
